...
function _PermisosTabla({ filas, webapp, etiqueta, colFecha, sinPendientesMsg }) {
  const [filtro,      setFiltro]      = _us('Pendientes');
  const [autorizando, setAutorizando] = _us(null);
  const [rows,        setRows]        = _us(filas);
  _ue(() => setRows(filas), [filas]);

  const autorizar = (row) => {
    setAutorizando(row.fila);
    fetch(`${webapp}?fila=${row.fila}`, { mode:'no-cors' })
      .finally(() => {
        setRows(prev => prev.map(r => r.fila === row.fila ? {...r, aprobado:true} : r));
        setAutorizando(null);
        window._refreshBadges?.();
      });
  };
                  ...
                  <tr key={row.fila}>
                    ...
                    <td style={{ textAlign:'center' }}>
                      {row.aprobado ? (
                        ...
                        <button onClick={() => autorizar(row)} disabled={autorizando===row.fila}
                          style={{
                            padding:'5px 14px', borderRadius:6, border:'none',
                            background: autorizando===row.fila ? 'var(--sf2)' : 'var(--ok)',
                            color:      autorizando===row.fila ? 'var(--t3)'  : '#fff',
                            fontWeight:600, fontSize:12,
                            cursor: autorizando===row.fila ? 'default' : 'pointer',
                          }}>
                          {autorizando===row.fila ? 'Autorizando…' : 'Autorizar'}
                        </button>
                      )}
                    </td>
                  </tr>
    ...
    <div className="fade-in">
      ...
      {loading ? (
        ...
        <>
          ...
          <_PermisosTabla filas={permisos} webapp={_PERM_WEBAPP}
        ...
        <>
          {/* 1 — Tabla de autorización */}
          <_PermisosTabla filas={faltas} webapp={_FALTAS_WEBAPP}
