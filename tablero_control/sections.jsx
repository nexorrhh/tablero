...
const _PERM_SHEET    = '1SA0AphMQCf5biub1EqkfL1HgSoseFgUUdbJjaZED5Ug';
const _PERM_GID      = '1524672113';
const _PERM_WEBAPP   = 'https://script.google.com/macros/s/AKfycbx8o1M9xefSUOQuQMOq2X9xw5E7VVDCWiZjKy4WM1MYq0ZZOsKJcjzAYSmIx0fssfz3Bg/exec';
const _FALTAS_SHEET  = '1hzYW7WnnZbc2v1B0WABrK8Slr0F5AE7h-j4GO4jeVEs';
const _FALTAS_GID    = '1140348609';
const _FALTAS_WEBAPP = 'https://script.google.com/macros/s/AKfycbyMQCmKL7AlOIaBXP4_OzOParAvhgz2XuRUnw9XTjzncM0Z2A50x8c8_G9suKydXKuAdQ/exec';

function _parsePermDate(v) {
  if (!v) return null;
  const m = String(v).match(/Date\((\d+),(\d+),(\d+)(?:,(\d+),(\d+))?/);
  if (!m) return null;
  return new Date(+m[1], +m[2], +m[3], +(m[4]||0), +(m[5]||0));
}

function _fmtPermFecha(d) {
  if (!d) return '—';
...
function RRHHPermisos({ permisos, loadingPermisos, faltas, loadingFaltas }) {
  const [seccion, setSeccion] = _us('permisos');
  const [periodo, setPeriodo] = _us('año');

  const now  = new Date();
  const anio = now.getFullYear();

  const hace30 = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);
  const _esPend = r => !r.aprobado && (!r.marca || r.marca >= hace30);

  const permPend = permisos.filter(_esPend).length;
  const faltPend = faltas.filter(_esPend).length;
  const loading  = seccion === 'permisos' ? loadingPermisos : loadingFaltas;

  const permFilt = periodo === 'año'
    ? permisos.filter(r => r.marca && r.marca.getFullYear() === anio)
    : permisos;
  const faltFilt = periodo === 'año'
    ? faltas.filter(r => r.marca && r.marca.getFullYear() === anio)
    : faltas;

  const periodoLbl = periodo === 'año' ? String(anio) : 'histórico';

  return (
    <div className="fade-in">
      {/* Pill-tabs + toggle período */}
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between',
                    flexWrap:'wrap', gap:8, marginBottom:20 }}>
        <div style={{ display:'flex', gap:8 }}>
          {[['permisos','Permisos',permPend],['faltas','Faltas',faltPend]].map(([id,lbl,pend]) => (
            <button key={id} onClick={() => setSeccion(id)}
              style={{
                padding:'7px 20px', borderRadius:20, border:'1px solid var(--bd)',
              ...
              {pend > 0 && (
                <span style={{ marginLeft:7, background:'rgba(255,255,255,0.3)',
                               borderRadius:10, padding:'1px 7px', fontSize:11 }}>{pend}</span>
              )}
            </button>
          ))}
        </div>
        <div style={{ display:'flex', gap:6 }}>
          {[['año', String(anio)], ['historico','Histórico']].map(([id,lbl]) => (
            <button key={id} onClick={() => setPeriodo(id)}
              style={{
                padding:'5px 14px', borderRadius:16, fontSize:12, cursor:'pointer',
                border:'1px solid var(--bd)', transition:'all .12s',
                background: periodo===id ? 'var(--sf1)' : 'var(--sf2)',
                color:      periodo===id ? 'var(--t1)'  : 'var(--t3)',
      ...
      {loading ? (
        ...
        <>
          {/* 1 — Tabla de autorización */}
          <_PermisosTabla filas={permisos}
            sheetUrl={`https://docs.google.com/spreadsheets/d/${_PERM_SHEET}/edit#gid=${_PERM_GID}`}
            etiqueta="Llegadas tarde y salidas anticipadas" colFecha="Fecha / hora" />

          {/* 2 — KPIs */}
          <div className="g4 mb24" style={{ marginTop:24 }}>
            <Kpi icon="inbox"  label="Pendientes"     value={permPend}
                 note="requieren autorización" color="var(--warn)" />
            <Kpi icon="check"  label="Autorizadas"    value={permFilt.filter(r=>r.aprobado).length}
                 note={periodoLbl} color="var(--ok)" />
            <Kpi icon="clock"  label="Llegadas tarde" value={permFilt.filter(r=>/llegada/i.test(r.tipo||'')).length}
                 note={periodoLbl} color="var(--accent)" />
            <Kpi icon="logout" label="Salidas antic."  value={permFilt.filter(r=>/salida/i.test(r.tipo||'')).length}
                 note={periodoLbl} color="#5aa9f5" />
          </div>
          ...
          {(() => {
            ...
            permFilt.forEach(r => {
              if (!r.nombre) return;
              const n = r.nombre;
              if (!rank[n]) rank[n] = { nombre:n, total:0, llegadas:0, salidas:0 };
              rank[n].total++;
              if (/llegada/i.test(r.tipo||'')) rank[n].llegadas++;
              if (/salida/i.test(r.tipo||''))  rank[n].salidas++;
            });
            const top = Object.values(rank).sort((a,b) => b.total - a.total);
            if (!top.length) return null;
            const maxTotal = top[0].total;
            return (
              <Card title={`Ranking por colaborador — ${periodoLbl}`} icon="users">
                {/* Gráfico de barras horizontal */}
                <div style={{ padding:'16px 20px 4px' }}>
                  {top.map(p => (
                    <div key={p.nombre} style={{ display:'flex', alignItems:'center', gap:10, marginBottom:9 }}>
                      <div style={{ width:150, fontSize:12, color:'var(--t1)', fontWeight:500,
                                    textAlign:'right', flexShrink:0,
                                    overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                        {p.nombre}
                      </div>
                      <div style={{ flex:1, height:22, background:'var(--sf2)', borderRadius:4,
                                    overflow:'hidden', position:'relative' }}>
                        <div style={{ position:'absolute', left:0, top:0, height:'100%',
                                      width:`${(p.llegadas/maxTotal)*100}%`,
                                      background:'var(--accent)', borderRadius:'4px 0 0 4px' }} />
                        <div style={{ position:'absolute', left:`${(p.llegadas/maxTotal)*100}%`, top:0, height:'100%',
                                      width:`${(p.salidas/maxTotal)*100}%`,
                                      background:'#5aa9f5' }} />
                      </div>
                      <div style={{ width:26, fontSize:12, fontWeight:700, color:'var(--t1)',
                                    flexShrink:0, textAlign:'right' }}>{p.total}</div>
                    </div>
                  ))}
                  <div style={{ display:'flex', gap:16, marginTop:10, marginBottom:8, fontSize:11, color:'var(--t3)' }}>
                    <span><span style={{ display:'inline-block',width:10,height:10,borderRadius:2,
                                         background:'var(--accent)',marginRight:5 }} />Llegadas tarde</span>
                    <span><span style={{ display:'inline-block',width:10,height:10,borderRadius:2,
                                         background:'#5aa9f5',marginRight:5 }} />Salidas anticipadas</span>
                  </div>
                </div>
                {/* Tabla detalle */}
                <div className="tbl-wrap">
                  <table>
                    <thead><tr>
                      ...
                      <th style={{textAlign:'right'}}>Llegadas tarde</th>
                      <th style={{textAlign:'right'}}>Salidas antic.</th>
                    </tr></thead>
                    <tbody>
                      {top.map(p => (
                        <tr key={p.nombre}>
                          ...
                          <td style={{textAlign:'right',color:'var(--accent)'}}>{p.llegadas||'—'}</td>
                          <td style={{textAlign:'right',color:'#5aa9f5'}}>{p.salidas||'—'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
        ...
        <>
          {/* 1 — Tabla de autorización */}
          <_PermisosTabla filas={faltas}
            sheetUrl={`https://docs.google.com/spreadsheets/d/${_FALTAS_SHEET}/edit#gid=${_FALTAS_GID}`}
            etiqueta="Solicitudes de falta" colFecha="Fecha ausencia"
            sinPendientesMsg="No hay faltas pendientes de autorización. ✓" />

          {/* 2 — KPIs */}
          <div className="g3 mb24" style={{ marginTop:24 }}>
            <Kpi icon="inbox"  label="Pendientes"  value={faltPend}
                 note="requieren autorización" color="var(--warn)" />
            <Kpi icon="check"  label="Autorizadas" value={faltFilt.filter(r=>r.aprobado).length}
                 note={periodoLbl} color="var(--ok)" />
            <Kpi icon="users"  label="Ausencias"   value={faltFilt.length}
                 note={periodoLbl} color="var(--accent)" />
          </div>
          ...
          {(() => {
            ...
            faltFilt.forEach(r => {
              if (!r.nombre) return;
              const n = r.nombre;
              if (!rank[n]) rank[n] = { nombre:n, total:0, autorizadas:0 };
              rank[n].total++;
              if (r.aprobado) rank[n].autorizadas++;
            });
            const top = Object.values(rank).sort((a,b) => b.total - a.total);
            if (!top.length) return null;
            const maxTotal = top[0].total;
              ...
              <Card title={`Ranking por colaborador — ${periodoLbl}`} icon="users">
                ...
                <div style={{ padding:'16px 20px 4px' }}>
                  {top.map(p => (
                    <div key={p.nombre} style={{ display:'flex', alignItems:'center', gap:10, marginBottom:9 }}>
                      ...
                      <div style={{ flex:1, height:22, background:'var(--sf2)', borderRadius:4, overflow:'hidden' }}>
                        <div style={{ height:'100%', width:`${(p.total/maxTotal)*100}%`,
                                      background:'var(--ok)', borderRadius:4, transition:'width .3s' }} />
                      </div>
                      <div style={{ width:26, fontSize:12, fontWeight:700, color:'var(--t1)',
                                    flexShrink:0, textAlign:'right' }}>{p.total}</div>
                    </div>
                ...
                <div className="tbl-wrap">
                  <table>
                    <thead><tr>
                      <th>Colaborador</th>
                      <th style={{textAlign:'right'}}>Total faltas</th>
                      <th style={{textAlign:'right'}}>Autorizadas</th>
                      <th style={{textAlign:'right'}}>Pendientes</th>
                    </tr></thead>
                    <tbody>
                      {top.map(p => {
                        const pend = p.total - p.autorizadas;
                        return (
                          <tr key={p.nombre}>
                            <td style={{fontWeight:600,color:'var(--t1)'}}>{p.nombre}</td>
                            <td style={{textAlign:'right',fontWeight:700}}>{p.total}</td>
                            <td style={{textAlign:'right',color:'var(--ok)'}}>{p.autorizadas||'—'}</td>
                            <td style={{textAlign:'right',color:pend>0?'var(--warn)':'var(--t3)',
                                        fontWeight:pend>0?700:400}}>{pend||'—'}</td>
                          </tr>
                        );
                      })}
                    </tbody>
...
window.ViewRRHH = function ViewRRHH({ tab, onTabChange }) {
  ...
  const [loading,          setLoading]          = _us(true);
  const [permisos,         setPermisos]         = _us([]);
  const [loadingPermisos,  setLoadingPermisos]  = _us(true);
  const [faltas,           setFaltas]           = _us([]);
  const [loadingFaltas,    setLoadingFaltas]    = _us(true);
  ...
  _ue(() => {
    fetch(`https://docs.google.com/spreadsheets/d/${_PERM_SHEET}/gviz/tq?tqx=out:json&gid=${_PERM_GID}`)
      .then(r => r.text())
      .then(txt => {
        const json = JSON.parse(txt.slice(txt.indexOf('{'), txt.lastIndexOf('}')+1));
        const parsed = (json.table?.rows || []).map((r, idx) => {
          const c = r.c || [];
          const tipo = c[7]?.v || '';
          const esLlegada = /llegada/i.test(tipo);
          return {
            fila:        idx + 2,
            marca:       _parsePermDate(c[0]?.v),
            nombre:      c[1]?.v || '',
            email:       c[5]?.v || '',
            tipo,
            fechaEvento: _parsePermDate(esLlegada ? c[3]?.v : c[8]?.v),
            regreso:     c[9]?.v || '',
            motivo:      esLlegada ? (c[4]?.v || '') : (c[10]?.v || ''),
            aprobado:    c[11]?.v === true,
          };
        }).filter(r => r.nombre && r.tipo);
        setPermisos(parsed);
        setLoadingPermisos(false);
      })
      .catch(() => setLoadingPermisos(false));
  }, []);

  _ue(() => {
    fetch(`https://docs.google.com/spreadsheets/d/${_FALTAS_SHEET}/gviz/tq?tqx=out:json&gid=${_FALTAS_GID}`)
      .then(r => r.text())
      .then(txt => {
        const json = JSON.parse(txt.slice(txt.indexOf('{'), txt.lastIndexOf('}')+1));
        const parsed = (json.table?.rows || []).map((r, idx) => {
          ...
          return {
            ...
            email:        c[1]?.v || '',
            nombre:       c[2]?.v || '',
            fechaAusencia:_parsePermDate(c[4]?.v),
            motivo:       c[5]?.v || '',
            aprobado:     c[7]?.v === true,
            ingresadoPor: c[8]?.v || '',
          };
      ...
      .catch(() => setLoadingFaltas(false));
  }, []);

  const permisosPendientes = (loadingPermisos || loadingFaltas) ? null
    : permisos.filter(r => !r.aprobado).length + faltas.filter(r => !r.aprobado).length;
  ...
  const views = [
    ...
    <RRHHPermisos permisos={permisos} loadingPermisos={loadingPermisos} faltas={faltas} loadingFaltas={loadingFaltas} />,
    <RRHHSabados />,
    <RRHHPostulantes />,
    <RRHHVencimientos />,
  ];
