      ...
      <div className="card-body" style={{ paddingBottom:4 }}>
        <svg viewBox={`0 0 ${VW} ${VH}`} width="100%" height={VH} style={{ display:'block', overflow:'visible' }}>
          ...
          {labels.map((label, i) => {
            ...
            // Construir segmentos crudos
            const rawSegs = datasets.map(ds => ({ v: ds.data[i] || 0, color: ds.color }));
            const stackTotal = rawSegs.reduce((s, r) => s + r.v, 0);
            // Si el stack supera el total de sheets, escalar proporcionalmente
            const ratio = stackTotal > postArr[i] && postArr[i] > 0 ? postArr[i] / stackTotal : 1;
            let cumV = 0;
            const segs = rawSegs.map(r => {
              const v = r.v * ratio;
              const seg = { v, h: toH(v), y: padT + cH - toH(cumV + v), color: r.color };
              cumV += v;
              return seg;
            });
            ...
            return (
              <g key={i}
                ...
                {postArr[i] > 0 && (
                  <text x={x + barW / 2} y={padT + cH - toH(postArr[i]) - 3}
                        textAnchor="middle" fontSize={8} fill="var(--t2)">{postArr[i]}</text>
                )}
...
function RRHHPanel({ onNavigate, empleados, loading, permisosPendientes }) {
  ...
  return (
    <div className="fade-in">
      ...
      <div className="g4">
        ...
        <Kpi icon="inbox"    label="Permisos pend."
             value={permisosPendientes === null ? '—' : String(permisosPendientes)}
             note={permisosPendientes > 0 ? 'requieren autorización' : 'al día'}
             color={permisosPendientes > 0 ? 'var(--warn)' : 'var(--ok)'} />
...
// Helper Nexo RRHH — llama a la Edge Function del tenant correcto
async function _nexoCall(tenant, action, body) {
  const cfg = window.NEXO_CONFIG?.[tenant];
  if (!cfg?.url || !cfg?.apiKey) throw new Error(`Nexo ${tenant} no configurado`);
  const res = await fetch(`${cfg.url}?action=${action}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ api_key: cfg.apiKey, ...body }),
  });
  const data = await res.json();
  if (!res.ok || data.error) throw new Error(data.error ?? 'Error Nexo');
  return data;
}

// Placeholder — reemplazado por RRHHPermisos nuevo abajo
function _PermisosTabla({ filas, sheetUrl, etiqueta, colFecha, sinPendientesMsg }) {
  const [filtro, setFiltro] = _us('Pendientes');
  const [rows,   setRows]   = _us(filas);
  _ue(() => setRows(filas), [filas]);

  const autorizar = (row) => {
    const url = sheetUrl ? `${sheetUrl}&range=A${row.fila}` : sheetUrl;
  ...
  const pendientes = rows.filter(r => !r.aprobado && (!r.marca || r.marca >= hace30));
  const estesMes   = rows.filter(r => r.marca && r.marca.getFullYear()===anio && r.marca.getMonth()===mes);
  const FILTROS    = ['Pendientes','Este mes','Histórico'];

  const filtradas = _um(() => {
    if (filtro === 'Pendientes') return pendientes;
    if (filtro === 'Este mes')   return estesMes;
    return rows;
  }, [rows, filtro]);
      ...
      <div className="card">
        <div className="card-header">
          <div className="card-title">
            <span className="card-title-icon"><Icon name="clipboard" size={15} /></span>
            {etiqueta}
          </div>
          <FilterChips options={FILTROS} active={filtro} onChange={setFiltro} />
        </div>
        {filtradas.length === 0 ? (
          <div style={{ padding:'48px', textAlign:'center', color:'var(--t2)', fontSize:13 }}>
            {filtro === 'Pendientes' ? (sinPendientesMsg || 'No hay solicitudes pendientes. ✓') : 'No hay registros en este período.'}
          </div>
        ) : (
                ...
                {filtradas.map(row => (
                  <tr key={row.fila}>
                    <td style={{ fontWeight:600, color:'var(--t1)' }}>
                      {row.nombre}
                      {row.tipo && <span style={{ marginLeft:8 }}><Badge tipo={/llegada/i.test(row.tipo)?'info':'warning'}>{row.tipo}</Badge></span>}
                      {row.ingresadoPor && <span style={{ marginLeft:8, fontSize:11, color:'var(--t3)' }}>{row.ingresadoPor}</span>}
                    </td>
                    {colFecha && (
                      <td style={{ color:'var(--t2)', fontSize:12, whiteSpace:'nowrap' }}>
                        {_fmtPermFecha(row.fechaEvento || row.fechaAusencia)}
                      </td>
                    )}
                    <td style={{ color:'var(--t2)', fontSize:12 }}>{row.motivo || '—'}</td>
                    <td>
                      {row.aprobado
                        ? <Badge tipo="success">Autorizada</Badge>
...
function RRHHPermisos({ permisos, loadingPermisos, errorPermisos, onRefresh }) {
  const [seccion,          setSeccion]         = _us('pendientes'); // 'pendientes' | 'config'
  const [expanded,         setExpanded]        = _us(null);
  const [accionExp,        setAccionExp]       = _us(null);
  const [obs,              setObs]             = _us('');
  const [saving,           setSaving]          = _us(false);
  const [saveErr,          setSaveErr]         = _us(null);
  const [toast,            setToast]           = _us(null);
  const [cfgData,          setCfgData]         = _us(null);
  const [cfgLoading,       setCfgLoading]      = _us(false);
  const [cfgSavingTenant,  setCfgSavingTenant] = _us(null);
  const [cfgInputs,        setCfgInputs]       = _us({ cimomet: '', comoing: '' });
  const [statsData,        setStatsData]       = _us(null);
  const [loadingStats,     setLoadingStats]    = _us(false);

  _ue(() => {
    if (seccion !== 'indicadores' || statsData !== null) return;
    setLoadingStats(true);
    Promise.all([
      _nexoCall('cimomet', 'get_estadisticas_permisos', {})
        .then(d => (d.permisos || []).map(p => ({ ...p, empresa: 'CIMOMET' })))
        .catch(() => []),
      _nexoCall('comoing', 'get_estadisticas_permisos', {})
        .then(d => (d.permisos || []).map(p => ({ ...p, empresa: 'COMOING' })))
        .catch(() => []),
    ]).then(([ci, co]) => {
      setStatsData([...ci, ...co]);
      setLoadingStats(false);
    }).catch(() => { setStatsData([]); setLoadingStats(false); });
  }, [seccion]);

  _ue(() => {
    if (seccion !== 'config' || cfgData) return;
    setCfgLoading(true);
    Promise.all([
      _nexoCall('cimomet', 'get_config_autorizadores', {}).catch(() => ({ emails: [] })),
      _nexoCall('comoing', 'get_config_autorizadores', {}).catch(() => ({ emails: [] })),
    ]).then(([ci, co]) => {
      setCfgData({ cimomet: [...(ci.emails || [])], comoing: [...(co.emails || [])] });
      setCfgLoading(false);
    });
  }, [seccion]);
  ...
  const tipoInfo = tipo => {
    if (tipo === 'salida_anticipada') return { label: 'Salida anticipada', color: 'var(--warn)' };
    if (tipo === 'ausencia')          return { label: 'Ausencia',           color: '#8b5cf6' };
    return                                   { label: 'Llegada tarde',      color: '#3b82f6' };
  };
  ...
  const fmtFechaPerm = (fecha, hora, tipo) => {
    if (!fecha) return '—';
    const [y, m, d] = fecha.split('-');
    const fs = `${d}/${m}/${y.slice(2)}`;
    if (tipo === 'ausencia') return `${fs} — jornada completa`;
    return hora ? `${fs} a las ${(hora || '').slice(0, 5)}` : fs;
  };

  const abrirAccion = (p, accion) => {
    if (expanded === p.id && accionExp === accion) { cancelar(); return; }
    setExpanded(p.id); setAccionExp(accion); setObs(''); setSaveErr(null);
  };

  const cancelar = () => { setExpanded(null); setAccionExp(null); setObs(''); setSaveErr(null); };

  const confirmar = async p => {
    if (accionExp === 'rechazar' && !obs.trim()) { setSaveErr('El comentario es requerido para rechazar.'); return; }
    setSaving(true); setSaveErr(null);
    try {
      const tenant = p.empresa === 'COMOING' ? 'comoing' : 'cimomet';
      const _u = window.getCurrentUser();
      await _nexoCall(tenant, 'autorizar_permiso', {
        permiso_id: p.id, accion: accionExp,
        observacion: obs.trim() || undefined,
        autorizado_por: _u?.name || null,
      });
      showToast(accionExp === 'aprobar' ? 'Permiso aprobado ✓' : 'Permiso rechazado', accionExp === 'aprobar' ? 'ok' : 'err');
      cancelar();
      onRefresh();
    } catch (e) {
      setSaveErr(e.message || 'Error al procesar');
    } finally {
      setSaving(false);
    }
  };

  const saveEmailsCfg = async tenant => {
    setCfgSavingTenant(tenant);
    try {
      await _nexoCall(tenant, 'set_config_autorizadores', { emails: cfgData[tenant] });
      showToast('Guardado ✓', 'ok');
    } catch (e) {
      showToast(e.message || 'Error al guardar', 'err');
    } finally {
      setCfgSavingTenant(null);
    }
  };

  const addEmail = tenant => {
    const val = cfgInputs[tenant].trim();
    if (!val || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(val)) return;
    if (cfgData[tenant].includes(val)) return;
    setCfgData(d => ({ ...d, [tenant]: [...d[tenant], val] }));
    setCfgInputs(i => ({ ...i, [tenant]: '' }));
  };

  const removeEmail = (tenant, email) =>
    setCfgData(d => ({ ...d, [tenant]: d[tenant].filter(e => e !== email) }));

  const stats = _um(() => {
    if (!statsData || !statsData.length) return null;
    const total      = statsData.length;
    const aprobados  = statsData.filter(p => p.estado === 'aprobado').length;
    const rechazados = statsData.filter(p => p.estado === 'rechazado').length;
    const pendientes = statsData.filter(p => p.estado === 'pendiente').length;
    const tasa = (aprobados + rechazados) > 0
      ? Math.round(aprobados / (aprobados + rechazados) * 100) : 0;
    const tiposCount = {
      llegada_tarde:     statsData.filter(p => p.tipo === 'llegada_tarde').length,
      salida_anticipada: statsData.filter(p => p.tipo === 'salida_anticipada').length,
      ausencia:          statsData.filter(p => p.tipo === 'ausencia').length,
    };
    const porEmpresa = {
      CIMOMET: statsData.filter(p => p.empresa === 'CIMOMET').length,
      COMOING: statsData.filter(p => p.empresa === 'COMOING').length,
    };
    const empMap = {};
    statsData.forEach(p => {
      const k = String(p.empleados?.legajo || '?');
      if (!empMap[k]) empMap[k] = {
        nombre: `${p.empleados?.apellido || ''}, ${p.empleados?.nombre || ''}`,
        legajo: k, count: 0,
      };
      empMap[k].count++;
    });
    const topEmpleados = Object.values(empMap).sort((a,b) => b.count - a.count).slice(0, 5);
    const semanas = [];
    const now = new Date();
    for (let i = 7; i >= 0; i--) {
      const wEnd   = new Date(now); wEnd.setDate(now.getDate() - i * 7);
      const wStart = new Date(wEnd); wStart.setDate(wEnd.getDate() - 7);
      const count  = statsData.filter(p => { const d = new Date(p.created_at); return d >= wStart && d < wEnd; }).length;
      semanas.push({ label: `${wStart.getDate()}/${wStart.getMonth()+1}`, count });
    }
    return { total, aprobados, rechazados, pendientes, tasa, tiposCount, porEmpresa, topEmpleados, semanas };
  }, [statsData]);
    ...
    <div className="fade-in">
      ...
      {toast && (
        <div style={{ position:'fixed', bottom:24, right:24, zIndex:9999, padding:'12px 20px',
                      ...
                      background: toast.tipo === 'ok' ? 'var(--ok)' : 'var(--err)',
                      color:'#fff', boxShadow:'0 4px 20px rgba(0,0,0,0.25)', transition:'opacity .2s' }}>
          {toast.msg}
        </div>
      )}
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between',
        ...
        <div style={{ display:'flex', gap:8 }}>
          {[['pendientes','Permisos pendientes', permisos.length],
            ['indicadores','Indicadores',         null],
            ['config',    'Avisos por correo',   null]].map(([id, lbl, cnt]) => (
            <button key={id} onClick={() => setSeccion(id)}
              style={{ padding:'7px 20px', borderRadius:20, border:'1px solid var(--bd)',
                       background: seccion===id ? 'var(--accent)' : 'var(--sf2)',
                       color: seccion===id ? '#fff' : 'var(--t2)',
                       fontWeight:600, fontSize:13, cursor:'pointer' }}>
              {lbl}
              {cnt > 0 && (
                <span style={{ marginLeft:7, background:'rgba(255,255,255,0.3)',
                               borderRadius:10, padding:'1px 7px', fontSize:11 }}>{cnt}</span>
              )}
            </button>
          ))}
        </div>
        {seccion === 'pendientes' && (
          <button onClick={onRefresh} disabled={loadingPermisos}
            style={{ padding:'6px 14px', borderRadius:8, border:'1px solid var(--bd)',
                     background:'var(--sf2)', color:'var(--t2)', fontSize:12,
                     cursor: loadingPermisos ? 'default' : 'pointer', opacity: loadingPermisos ? 0.5 : 1 }}>
            ↺ Actualizar
          </button>
        )}
      </div>
      ...
      {seccion === 'pendientes' ? (
        loadingPermisos ? (
          <div className="card" style={{ padding:'64px', textAlign:'center', color:'var(--t2)' }}>Cargando...</div>
        ) : errorPermisos ? (
          <div className="card" style={{ padding:'40px', textAlign:'center' }}>
            <div style={{ color:'var(--err)', marginBottom:12, fontSize:13 }}>
              No se pudo conectar con Nexo RRHH: {errorPermisos}
            </div>
          ...
          <div className="card" style={{ padding:'64px', textAlign:'center' }}>
            ...
            <div style={{ fontSize:13, color:'var(--t2)' }}>No hay permisos pendientes de aprobación.</div>
          </div>
        ) : (
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            {permisos.map(p => {
              ...
              return (
                <div key={p.id}
                  style={{ background:'var(--sf1)', border:'1px solid var(--bd)',
                  ...
                  <div style={{ padding:'12px 16px', display:'flex', alignItems:'center',
                    ...
                    <span style={{ padding:'3px 10px', borderRadius:20, fontSize:11, fontWeight:700,
                                   background: `${ti.color}22`, color: ti.color, flexShrink:0 }}>
                      {ti.label}
                    </span>
                    <Badge>{p.empresa}</Badge>
                    <div style={{ fontWeight:600, fontSize:13, color:'var(--t1)' }}>
                      {p.empleado.apellido}, {p.empleado.nombre}
                      <span style={{ marginLeft:8, fontSize:11, fontWeight:400, color:'var(--t3)' }}>
                        Leg. {p.empleado.legajo}
                      </span>
                    </div>
                    <div style={{ fontSize:12, color:'var(--t2)' }}>
                      {fmtFechaPerm(p.fecha, p.hora, p.tipo)}
                    </div>
                    {p.motivo && (
                    ...
                    <div style={{ marginLeft:'auto', fontSize:11, color:'var(--t3)', flexShrink:0 }}>
                      {haceTiempo(p.created_at)}
                    </div>
                    <div style={{ display:'flex', gap:6, flexShrink:0 }}>
                      <button onClick={() => abrirAccion(p, 'aprobar')}
                        style={{ padding:'5px 14px', borderRadius:6, border:'none', fontWeight:600,
                                 fontSize:12, cursor:'pointer',
                                 background: isExp && accionExp==='aprobar' ? 'var(--ok)' : 'rgba(22,163,74,0.15)',
                                 color:      isExp && accionExp==='aprobar' ? '#fff' : 'var(--ok)' }}>
                        Aprobar
                      </button>
                      <button onClick={() => abrirAccion(p, 'rechazar')}
                        style={{ padding:'5px 14px', borderRadius:6, border:'none', fontWeight:600,
                                 fontSize:12, cursor:'pointer',
                                 background: isExp && accionExp==='rechazar' ? 'var(--err)' : 'rgba(239,68,68,0.12)',
                                 color:      isExp && accionExp==='rechazar' ? '#fff' : 'var(--err)' }}>
                        Rechazar
                      </button>
                    </div>
                  </div>
                  ...
                  {isExp && (
                    <div style={{ borderTop:'1px solid var(--bd)', padding:'14px 16px',
                                  background:'var(--sf2)' }}>
                      <div style={{ marginBottom:8, fontSize:12, color:'var(--t2)', fontWeight:600 }}>
                        {accionExp === 'rechazar' ? 'Motivo del rechazo *' : 'Observación (opcional)'}
                      </div>
                      <textarea value={obs} onChange={e => setObs(e.target.value)} rows={2}
                        placeholder={accionExp === 'rechazar' ? 'Explicá el motivo...' : 'Podés agregar un comentario...'}
                        style={{ width:'100%', borderRadius:6, border:'1px solid var(--bd)',
                                 background:'var(--sf1)', color:'var(--t1)', padding:'8px 10px',
                                 fontSize:12, resize:'vertical', fontFamily:'inherit',
                                 boxSizing:'border-box' }} />
                      {saveErr && (
                        <div style={{ fontSize:12, color:'var(--err)', marginTop:6 }}>{saveErr}</div>
                      )}
                      <div style={{ display:'flex', gap:8, marginTop:10 }}>
                        <button onClick={() => confirmar(p)} disabled={saving}
                          style={{ padding:'7px 18px', borderRadius:6, border:'none', fontWeight:600,
                                   fontSize:12, cursor:'pointer', opacity: saving ? 0.6 : 1,
                                   background: accionExp==='aprobar' ? 'var(--ok)' : 'var(--err)',
                                   color:'#fff' }}>
                          {saving ? 'Procesando...' : 'Confirmar'}
                        </button>
                        <button onClick={cancelar} disabled={saving}
                          style={{ padding:'7px 14px', borderRadius:6, border:'1px solid var(--bd)',
                                   background:'var(--sf1)', color:'var(--t2)', fontSize:12, cursor:'pointer' }}>
                          Cancelar
                        </button>
                      </div>
                    </div>
        ...
        loadingStats || !stats ? (
          <div className="card" style={{ padding:'64px', textAlign:'center', color:'var(--t2)' }}>
            {loadingStats ? 'Cargando estadísticas...' : 'Sin datos en los últimos 90 días.'}
          </div>
        ) : (() => {
          const maxSem = Math.max(...stats.semanas.map(s => s.count), 1);
          return (
            <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
              {/* KPIs */}
              <div className="g4">
                <Kpi icon="inbox"    label="SOLICITUDES 90D"  value={stats.total}     note="últimos 90 días"           color="var(--info)" />
                <Kpi icon="medal"    label="APROBADOS"        value={stats.aprobados} note={`Tasa: ${stats.tasa}%`}    color="var(--ok)"   />
                <Kpi icon="alert"    label="RECHAZADOS"       value={stats.rechazados} note="con motivo de rechazo"    color="var(--err)"  />
                <Kpi icon="calendar" label="PENDIENTES"       value={stats.pendientes} note="sin resolver aún"         color="var(--warn)" />
              </div>
              {/* Tendencia semanal */}
              <Card title="Tendencia semanal (últimas 8 semanas)" icon="calendar">
                <div style={{ padding:'8px 22px 20px' }}>
                  <div style={{ display:'flex', alignItems:'flex-end', gap:5, height:120 }}>
                    {stats.semanas.map((s, i) => {
                      const h = Math.max(3, Math.round((s.count / maxSem) * 96));
                      const isLast = i === stats.semanas.length - 1;
                      return (
                        <div key={i} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:4 }}>
                          {s.count > 0 && <div style={{ fontSize:10, color:'var(--t3)', fontWeight:600 }}>{s.count}</div>}
                          <div style={{ width:'100%', height:`${h}px`, borderRadius:'4px 4px 0 0',
                                        background: isLast ? 'var(--accent)' : 'rgba(224,33,138,0.28)',
                                        transition:'height .4s ease' }} />
                          <div style={{ fontSize:9, color:'var(--t3)', whiteSpace:'nowrap' }}>{s.label}</div>
                        </div>
                      );
                    })}
                  </div>
              ...
              {/* Por tipo + Top empleados */}
              <div className="g2">
                <Card title="Por tipo de solicitud" icon="medal">
                  <div className="card-body">
                    {[
                      { key:'llegada_tarde',     label:'Llegada tarde',     color:'#3b82f6' },
                      { key:'salida_anticipada', label:'Salida anticipada', color:'var(--warn)' },
                      { key:'ausencia',          label:'Ausencia',          color:'#8b5cf6' },
                    ].map(({ key, label, color }) => (
                      <div key={key} style={{ marginBottom:18 }}>
                        <div style={{ display:'flex', justifyContent:'space-between', marginBottom:5 }}>
                          <span style={{ fontSize:12, color:'var(--t2)' }}>{label}</span>
                          <span style={{ fontSize:12, fontWeight:600, color:'var(--t1)' }}>
                            {stats.tiposCount[key]}
                            <span style={{ color:'var(--t3)', fontWeight:400, marginLeft:5 }}>
                              ({stats.total ? Math.round(stats.tiposCount[key]/stats.total*100) : 0}%)
                            </span>
                          </span>
                        </div>
                        <div className="prog">
                          <div className="prog-fill" style={{ width:`${stats.total ? stats.tiposCount[key]/stats.total*100 : 0}%`, background:color }} />
                        </div>
                      </div>
                    ))}
                    <div style={{ marginTop:20, paddingTop:16, borderTop:'1px solid var(--bd)',
                                  display:'flex', gap:12 }}>
                      {[['CIMOMET','var(--info)'],['COMOING','var(--accent)']].map(([emp, color]) => (
                        <div key={emp} style={{ flex:1, textAlign:'center', padding:'10px 0',
                                                background:'var(--sf2)', borderRadius:10 }}>
                          <div style={{ fontSize:10, color:'var(--t3)', textTransform:'uppercase',
                                        letterSpacing:'0.06em', marginBottom:4 }}>{emp}</div>
                          <div style={{ fontSize:30, fontWeight:700, color,
                                        fontFamily:'DM Serif Display,serif' }}>{stats.porEmpresa[emp]}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                </Card>
                <Card title="Empleados más frecuentes" icon="users">
                  <div className="card-body">
                    {stats.topEmpleados.length === 0 ? (
                      <div style={{ color:'var(--t3)', fontSize:12, padding:'16px 0' }}>Sin datos aún.</div>
                    ) : stats.topEmpleados.map((e, i) => (
                      <div key={e.legajo}
                        style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 0',
                                 borderBottom: i < stats.topEmpleados.length-1 ? '1px solid var(--bd)' : 'none' }}>
                        <div style={{ width:22, fontSize:11, color:'var(--t3)', fontWeight:700, flexShrink:0 }}>
                          #{i+1}
                        </div>
                        <div style={{ flex:1, minWidth:0 }}>
                          ...
                          <div style={{ fontSize:11, color:'var(--t3)' }}>Leg. {e.legajo}</div>
                        </div>
        ...
        /* CONFIG SECTION */
        cfgLoading || !cfgData ? (
          <div className="card" style={{ padding:'40px', textAlign:'center', color:'var(--t2)' }}>Cargando...</div>
        ) : (
          <div style={{ display:'grid', gap:20, gridTemplateColumns:'repeat(auto-fit,minmax(300px,1fr))' }}>
            {[['cimomet','Cimomet S.A.'],['comoing','Co.mo.ing S.R.L.']].map(([tenant, nombre]) => (
              <div key={tenant} className="card">
                <div className="card-header">
                  <div className="card-title">
                    <span className="card-title-icon"><Icon name="inbox" size={15} /></span>
                    {nombre}
                  </div>
                </div>
                <div style={{ padding:'0 20px 20px' }}>
                  <div style={{ fontSize:12, color:'var(--t3)', marginBottom:12 }}>
                    Reciben aviso de nuevas solicitudes de permiso:
                  </div>
                  <div style={{ display:'flex', flexWrap:'wrap', gap:6, marginBottom:12, minHeight:32 }}>
                    {cfgData[tenant].length === 0 ? (
                      <span style={{ fontSize:12, color:'var(--t3)', fontStyle:'italic' }}>Sin correos configurados</span>
                    ) : cfgData[tenant].map(email => (
                      <span key={email}
                        style={{ display:'inline-flex', alignItems:'center', gap:5, padding:'4px 10px',
                                 borderRadius:16, background:'var(--sf2)', border:'1px solid var(--bd)',
                                 fontSize:12, color:'var(--t1)' }}>
                        {email}
                        <button onClick={() => removeEmail(tenant, email)}
                          style={{ background:'none', border:'none', cursor:'pointer',
                                   color:'var(--t3)', fontSize:15, lineHeight:1, padding:0 }}>
                          ×
                        </button>
                      </span>
                    ))}
                  ...
                  <div style={{ display:'flex', gap:6, marginBottom:14 }}>
                    <input value={cfgInputs[tenant]}
                      onChange={e => setCfgInputs(i => ({ ...i, [tenant]: e.target.value }))}
                      onKeyDown={e => e.key === 'Enter' && addEmail(tenant)}
                    ...
                    <button onClick={() => addEmail(tenant)}
                      style={{ padding:'7px 14px', borderRadius:6, border:'1px solid var(--bd)',
                               background:'var(--sf2)', color:'var(--t1)', fontSize:12,
                               cursor:'pointer', fontWeight:600 }}>
                      Agregar
                    </button>
                  </div>
                  <button onClick={() => saveEmailsCfg(tenant)} disabled={cfgSavingTenant === tenant}
                    style={{ padding:'8px 20px', borderRadius:8, border:'none',
                             background:'var(--accent)', color:'#fff', fontWeight:600,
                             fontSize:12, cursor:'pointer',
                             opacity: cfgSavingTenant === tenant ? 0.6 : 1 }}>
                    {cfgSavingTenant === tenant ? 'Guardando...' : 'Guardar cambios'}
                  </button>
                </div>
              </div>
            ))}
...
const AREAS_SP    = ['Administrativo','Calidad','Gerencia','Ingeniería','Producción','Taller'];
const _inputSP    = {
...
function FormularioSolicitud({ onSuccess }) {
  ...
  const [puestosOpts, setPuestosOpts] = _us(null); // null = cargando
  ...
  _ue(() => {
    ...
    fetch(`${url}/empleados?select=desc_puesto&activo=eq.true`, {
      ...
      .then(r => r.json())
      .then(data => {
        if (!Array.isArray(data)) { setPuestosOpts([]); return; }
        const uniq = [...new Set(data.map(e => e.desc_puesto).filter(Boolean))].sort();
        setPuestosOpts([{val:'',label:'— Seleccioná un puesto —'}, ...uniq.map(p => ({val:p,label:p}))]);
      })
      .catch(() => setPuestosOpts([]));
  }, []);
      ...
      <div style={{ padding:'0 20px 24px', display:'flex', flexDirection:'column', gap:14 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
          <div>
            ...
            <Select value={form.puesto} onChange={v => set('puesto', v)}
                    options={puestosOpts ?? [{val:'',label:'Cargando puestos...'}]} />
          </div>
          <div>
            <label style={lbl}>Area / Sector *</label>
            <Select value={form.area} onChange={v => set('area', v)}
                    options={[{val:'',label:'— Seleccioná un área —'}, ...AREAS_SP.map(a => ({val:a,label:a}))]} />
          </div>
        </div>
...
function PanelDirector({ pendientes, historial, onRefresh }) {
  ...
  const [empresaSel, setEmpresaSel] = _us('');
  const [saving,     setSaving]     = _us(false);
  ...
  const cerrar = () => { setModal(null); setNombre(window.getCurrentUser()?.name || ''); setNota(''); setEmpresaSel(''); setErrModal(null); };

  const confirmar = async () => {
    if (!nombre.trim()) return;
    const necesitaEmpresa = modal.accion === 'aprobado' && !modal.sol.empresa;
    if (necesitaEmpresa && !empresaSel) {
      setErrModal('Seleccioná la empresa antes de aprobar.');
      return;
    }
    setSaving(true);
    setErrModal(null);
    ...
    try {
      ...
      if (necesitaEmpresa) patch.empresa = empresaSel;
                ...
                <div style={{ display:'flex', justifyContent:'space-between',
                  ...
                  <div style={{ flex:1 }}>
                    <div style={{ display:'flex', gap:8, alignItems:'center',
                      ...
                      {s.empresa ? <Badge>{s.empresa}</Badge> : <Badge tipo="warning">Sin definir</Badge>}
                ...
                <tbody>
                  {historial.map(s => (
                    <tr key={s.id}>
                      ...
                      <td>{s.empresa ? <Badge>{s.empresa}</Badge> : <Badge tipo="warning">Sin definir</Badge>}</td>
          ...
          <div style={{ background:'var(--sf1)', border:'1px solid var(--bd)', borderRadius:12,
            ...
            <div style={{ padding:20, display:'flex', flexDirection:'column', gap:12 }}>
              ...
              <div style={{ background:'var(--sf2)', borderRadius:8,
                ...
                <b>{modal.sol.puesto}</b> · {modal.sol.area} ·{' '}
                {modal.sol.empresa
                  ? <Badge>{modal.sol.empresa}</Badge>
                  : <Badge tipo="warning">Sin definir</Badge>}
              </div>

              {/* Selector de empresa — solo cuando viene vacío y es aprobación */}
              {modal.accion === 'aprobado' && !modal.sol.empresa && (
                <div>
                  <label style={{ fontSize:11, color:'var(--warn)', display:'block',
                  ...
                  <div style={{ display:'flex', gap:8 }}>
                    {EMPRESAS_SP.map(emp => (
                      <button key={emp.val} onClick={() => setEmpresaSel(emp.val)}
                              style={{ flex:1, padding:'9px 12px', borderRadius:7, fontSize:13,
                                       fontWeight: empresaSel === emp.val ? 700 : 400,
                                       border: `2px solid ${empresaSel === emp.val ? 'var(--accent)' : 'var(--bd)'}`,
                                       background: empresaSel === emp.val ? 'rgba(224,33,138,0.1)' : 'var(--sf2)',
                                       color: empresaSel === emp.val ? 'var(--accent)' : 'var(--t2)',
              ...
              <div style={{ display:'flex', gap:8, justifyContent:'flex-end' }}>
                ...
                {(() => {
                  const necesitaEmpresa = modal.accion === 'aprobado' && !modal.sol.empresa;
                  const disabled = saving || !nombre.trim() || (necesitaEmpresa && !empresaSel);
                  return (
                    <button onClick={confirmar} disabled={disabled}
          ...
          <div style={{ padding:'12px 20px 16px', overflowX:'auto' }}>
            <svg width={svgW} height={svgH} style={{ display:'block', minWidth:'100%' }}>
              ...
              {dataRef.current.meses.map((m,i) => {
                ...
                const rawP  = dataRef.current.preselArr[i];
                const rawD  = dataRef.current.descartArr[i];
                const rawS  = Math.max(0, t - rawP - rawD);
                const total = rawP + rawD + rawS;
                const pPx   = total===0 ? 0 : Math.round((rawP/total)*tH);
                const dPx   = total===0 ? 0 : Math.round((rawD/total)*tH);
                ...
                return (
                  <tr key={s.id}>
                    <td>
                      ...
                      <div className="f12 t3">{s.area} · {s.empresa ? <Badge>{s.empresa}</Badge> : <Badge tipo="warning">Sin definir</Badge>}</div>
                    </td>
                    <td style={{ textAlign:'center' }}>
...
window.ViewRRHH = function ViewRRHH({ tab, onTabChange }) {
  ...
  const [loading,          setLoading]          = _us(true);
  const [nexoPermisos,    setNexoPermisos]    = _us([]);
  const [loadingPermisos, setLoadingPermisos] = _us(true);
  const [errorPermisos,   setErrorPermisos]   = _us(null);
  ...
  const _cargarPermisos = React.useCallback(() => {
    setLoadingPermisos(true);
    setErrorPermisos(null);
    Promise.all([
      _nexoCall('cimomet', 'get_permisos_pendientes', {})
        .then(d => (d.permisos || []).map(p => ({ ...p, empresa: 'CIMOMET' })))
        .catch(() => []),
      _nexoCall('comoing', 'get_permisos_pendientes', {})
        .then(d => (d.permisos || []).map(p => ({ ...p, empresa: 'COMOING' })))
        .catch(() => []),
    ])
      .then(([ci, co]) => {
        const merged = [...ci, ...co].sort((a, b) => new Date(a.created_at) - new Date(b.created_at));
        setNexoPermisos(merged);
        setLoadingPermisos(false);
      })
      .catch(e => { setErrorPermisos(e.message || 'Error'); setLoadingPermisos(false); });
  }, []);

  _ue(() => { _cargarPermisos(); }, []);

  const permisosPendientes = loadingPermisos ? null : nexoPermisos.length;

  const views = [
    <RRHHPanel onNavigate={onTabChange} empleados={empleados} loading={loading} permisosPendientes={permisosPendientes} />,
    <RRHHPlantel empleados={empleados} loading={loading} />,
    <RRHHPermisos permisos={nexoPermisos} loadingPermisos={loadingPermisos}
                  errorPermisos={errorPermisos} onRefresh={_cargarPermisos} />,
    <RRHHSabados />,
    <RRHHPostulantes />,
    <RRHHVencimientos />,
  ];
...
function _badgeVencInst(fecha, pravisoMeses) {
  const d = _diasHasta(fecha);
  const umbral = (pravisoMeses || 1) * 30;
  if (d < 0)       return <Badge tipo="danger">Vencido</Badge>;
  if (d <= umbral) return <Badge tipo="warning">Vence en {d}d</Badge>;
  return            <Badge tipo="success">Vigente</Badge>;
}

function ResumenVencInst({ items }) {
  const vencidos = items.filter(r => _diasHasta(r.fecha_vencimiento) < 0).length;
  const proximos = items.filter(r => {
    const d = _diasHasta(r.fecha_vencimiento);
    return d >= 0 && d <= (r.preaviso_meses || 1) * 30;
  }).length;
...
function RRHHVencimientos() {
  ...
  _ue(() => {
    ...
    fetch(`${url}/vencimientos_institucionales?order=fecha_vencimiento.asc`, {
      headers: { apikey: key, Authorization: `Bearer ${key}` },
    })
      .then(r => r.json())
      .then(d => { setInstitucionales(Array.isArray(d) ? d : []); setLoadI(false); })
      .catch(() => { setErrI(true); setLoadI(false); });
  }, []);
  ...
  if (subtab === 'contratos') {
    ...
    if (loadI)
      content = <div className="card" style={{ textAlign:'center', padding:'40px', color:'var(--t2)' }}>Cargando vencimientos institucionales...</div>;
    else if (errI)
      content = <div className="card" style={{ textAlign:'center', padding:'32px', color:'var(--err)' }}>No se pudieron cargar los datos.</div>;
    else if (institucionales.length === 0)
      content = <div className="card" style={{ textAlign:'center', padding:'32px', color:'var(--t2)' }}>No hay registros institucionales.</div>;
    else
      content = (
        <div className="card">
          <div className="card-header">
            <div className="card-title"><Icon name="medal" size={15} />Vencimientos institucionales</div>
          </div>
          <ResumenVencInst items={institucionales} />
          <div className="tbl-wrap">
            <table>
              <thead><tr>
                <th>Certificación / Habilitación</th>
                <th>Vencimiento</th>
                <th>Preaviso</th>
                <th>Notas</th>
                <th>Estado</th>
              ...
              <tbody>
                {institucionales.map(r => (
                  <tr key={r.id}>
                    <td className="cell-strong">{r.titulo}</td>
                    <td className="fw5">{_fmtFechaAR(r.fecha_vencimiento)}</td>
                    <td className="t2 f12">{r.preaviso_meses} {r.preaviso_meses === 1 ? 'mes' : 'meses'}</td>
                    <td className="t2 f12" style={{ maxWidth:260, whiteSpace:'normal' }}>{r.notas || '—'}</td>
                    <td>{_badgeVencInst(r.fecha_vencimiento, r.preaviso_meses)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="card" style={{ padding:'10px 16px', marginBottom:16 }}>
        <div style={{ display:'flex', gap:8 }}>
          {[
            ...
            { id:'institucionales', label:'Institucional'  },
          ].map(v => (
