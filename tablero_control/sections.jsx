...
function _PermisosTabla({ filas, webapp, etiqueta, colFecha, sinPendientesMsg }) {
  ...
  const pendientes = rows.filter(r => !r.aprobado);
...
function RRHHPermisos({ permisos, loadingPermisos, faltas, loadingFaltas }) {
  ...
  const permPend  = permisos.filter(r => !r.aprobado).length;
  const permMes   = permisos.filter(r => r.marca && r.marca.getFullYear()===anio && r.marca.getMonth()===mes);
  const llegadas  = permMes.filter(r => /llegada/i.test(r.tipo)).length;
  const salidas   = permMes.filter(r => /salida/i.test(r.tipo)).length;

  const faltPend  = faltas.filter(r => !r.aprobado).length;
  const faltMes   = faltas.filter(r => r.marca && r.marca.getFullYear()===anio && r.marca.getMonth()===mes);
  ...
  return (
    <div className="fade-in">
      ...
      <div style={{ display:'flex', gap:8, marginBottom:20 }}>
        {[['permisos','Permisos'], ['faltas','Faltas']].map(([id, lbl]) => (
          <button key={id} onClick={() => setSeccion(id)}
            ...
            {lbl}
            {id==='permisos' && permPend > 0 && (
              <span style={{ marginLeft:7, background:'rgba(255,255,255,0.3)', borderRadius:10,
                             padding:'1px 7px', fontSize:11 }}>{permPend}</span>
            )}
            {id==='faltas' && faltPend > 0 && (
              <span style={{ marginLeft:7, background:'rgba(255,255,255,0.3)', borderRadius:10,
                             padding:'1px 7px', fontSize:11 }}>{faltPend}</span>
            )}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <>
          <div className="g4 mb24">
            <Kpi icon="inbox"  label="Pendientes"     value={permPend}                        note="requieren autorización" color="var(--warn)"   />
            <Kpi icon="check"  label="Autorizadas"    value={permMes.filter(r=>r.aprobado).length} note="este mes"          color="var(--ok)"     />
            <Kpi icon="clock"  label="Llegadas tarde" value={llegadas}                        note="este mes"               color="var(--accent)" />
            <Kpi icon="logout" label="Salidas antic." value={salidas}                         note="este mes"               color="#5aa9f5"       />
          </div>
        ...
        <>
          <div className="g3 mb24">
            <Kpi icon="inbox"  label="Pendientes"   value={faltPend}                          note="requieren autorización" color="var(--warn)" />
            <Kpi icon="check"  label="Autorizadas"  value={faltMes.filter(r=>r.aprobado).length} note="este mes"            color="var(--ok)"   />
            <Kpi icon="users"  label="Ausencias"    value={faltMes.length}                    note="registradas este mes"   color="var(--accent)" />
          </div>
