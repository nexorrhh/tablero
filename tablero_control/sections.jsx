...
function PostulantesChart() {
  ...
  const [hoverIdx, setHoverIdx] = _us(null);
          ...
          {labels.map((label, i) => {
            ...
            return (
              <g key={i} onMouseEnter={() => setHoverIdx(i)} onMouseLeave={() => setHoverIdx(null)}>
                ...
                {isHov && postArr[i] > 0 && (
                  <text x={x + barW / 2} y={barTop - 4} textAnchor="middle" fontSize={9} fill="var(--t1)" fontWeight="700">{postArr[i]}</text>
                )}
    ...
    <div className="fade-in">
      <div className="g2 mb24">
        ...
        <div>
          ...
          <HubButton icon="clipboard" title="Permisos y solicitudes" subtitle="Próximamente"
                     onClick={() => onNavigate(2)} />
          <HubButton icon="calendar"  title="Sábados y feriados"
                     onClick={() => onNavigate(3)} />
          <HubButton icon="userplus"  title="Postulantes y candidatos"
...
function BusquedasRRHH({ busquedas, onRefresh }) {
  ...
  const cambiar = async (id, nuevoEstado) => {
    ...
    const { url, key } = window.SUPABASE_CONFIG;
    try {
      await fetch(`${url}/solicitudes_personal?id=eq.${id}`, {
        method: 'PATCH',
        headers: {
          apikey: key, Authorization: `Bearer ${key}`,
          'Content-Type': 'application/json', Prefer: 'return=minimal',
  ...
  _ue(() => {
    ...
    (async () => {
      try {
        ...
        /* ── Supabase: preseleccionados / descartados ── */
        const { url: sUrl, key } = window.SUPABASE_CONFIG;
        const hace12 = new Date(hoy.getFullYear(), hoy.getMonth()-11, 1).toISOString();
        const preselResp = await fetch(
          `${sUrl}/preseleccionados?select=estado,created_at&created_at=gte.${hace12}`,
        ...
        const preselData = await preselResp.json();
        const porMesPresel = new Map();
        if (Array.isArray(preselData)) {
          preselData.forEach(d => {
            const dt = new Date(d.created_at);
            const k  = `${dt.getFullYear()}-${dt.getMonth()}`;
            const e  = porMesPresel.get(k) || { activo:0, descartado:0 };
            if (d.estado === 'activo')      e.activo++;
            else if (d.estado === 'descartado') e.descartado++;
            porMesPresel.set(k, e);
          });
        }
        ...
        const postArr    = meses.map(m => porMesSheets.get(m.key) ?? 0);
        const preselArr  = meses.map(m => porMesPresel.get(m.key)?.activo ?? 0);
        const descartArr = meses.map(m => porMesPresel.get(m.key)?.descartado ?? 0);
        const sinProcArr = meses.map((_,i) => Math.max(0, postArr[i]-preselArr[i]-descartArr[i]));
...
function RRHHPostulantes() {
  ...
  _ue(() => {
    setLoading(true);
    const { url, key } = window.SUPABASE_CONFIG;
    fetch(`${url}/solicitudes_personal?order=created_at.desc`, {
      headers: { apikey:key, Authorization:`Bearer ${key}` }
    })
      .then(r => r.json())
        ...
        : vista === 'solicitud'
          ...
          : vista === 'director'
            ...
            : vista === 'panel'
              ...
              : <BusquedasRRHH busquedas={busquedas} onRefresh={refresh} />
      }
    </div>
  );
}
