...
/* Dropdown custom — renderiza via portal en <body> para evitar que cualquier
   transform/overflow/will-change de un padre rompa el posicionamiento */
function SelDrop({ value, onChange, options }) {
  const [open,  setOpen]  = _us(false);
  const [coord, setCoord] = _us(null);
  const btnRef = React.useRef(null);

  const current = options.find(o => o.val === value);
  const label   = current ? current.label : (options[0] && options[0].label) || '—';
  ...
  /* Portal: se monta directamente en document.body, fuera de toda jerarquía */
  const portal = (open && coord) ? ReactDOM.createPortal(
    <React.Fragment>
      ...
      <div style={{ position:'fixed',
                    ...
                    borderRadius:8, padding:4, zIndex:9001,
        ...
        {options.map(o => (
          <div key={o.val}
               ...
               style={{ padding:'8px 12px', borderRadius:6, fontSize:12,
                        ...
                        background: o.val === value ? 'rgba(224,33,138,0.15)' : 'transparent',
                        color:      o.val === value ? '#e0218a' : 'inherit' }}>
            {o.label}
          </div>
        ))}
      </div>
    ...
    <div style={{ position:'relative', display:'inline-block' }}>
      <button
        ...
        style={{ display:'flex', alignItems:'center', gap:8,
                 ...
                 fontSize:12, color:'inherit', cursor:'pointer',
                 whiteSpace:'nowrap', minWidth:130 }}>
        <span style={{ flex:1, textAlign:'left' }}>{label}</span>
        <span style={{ opacity:0.45, fontSize:9, lineHeight:1 }}>▼</span>
      </button>
      {portal}
    </div>
  );
  ...
  return (
    <div className="fade-in">
      ...
      <div className="card" style={{ padding:'10px 16px', marginBottom:12,
        ...
        <div style={{ borderLeft:'1px solid rgba(255,255,255,0.1)', paddingLeft:12, display:'flex', gap:8 }}>
          <SelDrop
            value={año}
            onChange={handleAño}
      ...
      {selected && (
        <div onClick={e => { if (e.target === e.currentTarget) setSelected(null); }}
             style={{ position:'fixed', inset:0, zIndex:1000,
                      ...
                      padding:24 }}>
          <div style={{ background:'#16111e', border:'1px solid rgba(255,255,255,0.1)',
                    ...
                    <thead>
                      <tr style={{ borderBottom:'1px solid rgba(255,255,255,0.07)' }}>
                        <th style={{ padding:'8px 12px', textAlign:'left', fontSize:11,
                                     color:'rgba(255,255,255,0.4)', fontWeight:500, width:48 }}></th>
                        <th style={{ padding:'8px 12px', textAlign:'left', fontSize:11,
                                     color:'rgba(255,255,255,0.4)', fontWeight:500 }}>Nombre</th>
                        <th style={{ padding:'8px 12px', textAlign:'left', fontSize:11,
                                     color:'rgba(255,255,255,0.4)', fontWeight:500 }}>Empresa</th>
                        <th style={{ padding:'8px 12px', textAlign:'left', fontSize:11,
                                     color:'rgba(255,255,255,0.4)', fontWeight:500 }}>Puesto</th>
                        <th style={{ padding:'8px 12px', textAlign:'left', fontSize:11,
                                     color:'rgba(255,255,255,0.4)', fontWeight:500 }}>Turno</th>
                        <th style={{ padding:'8px 12px', textAlign:'left', fontSize:11,
                                     color:'rgba(255,255,255,0.4)', fontWeight:500 }}>Situación</th>
                      </tr>
                    </thead>
                    <tbody>
                      {citados.map(d => (
                        <tr key={d.id}
                          ...
                          <td style={{ padding:'8px 12px', fontSize:12, color:'rgba(255,255,255,0.6)' }}>
                            {d.desc_puesto}
                          </td>
