...
function LoginScreen({ onLogin }) {
  ...
  const hasPin = uid => !!localStorage.getItem(`cmt_pin_${uid}`);
  ...
  const submitPin = (val, currentStep, currentFirst, currentSelected) => {
    if (currentStep === 'create') {
      ...
      if (val === currentFirst) {
        localStorage.setItem(`cmt_pin_${currentSelected}`, val);
      ...
      const stored = localStorage.getItem(`cmt_pin_${currentSelected}`);
      if (val === stored) {
        doLogin(currentSelected);
      } else {
  ...
  const addDigit = (d) => {
    ...
    setError('');
    if (next.length === 4) {
      setTimeout(() => submitPin(next, step, firstPin, selected), 150);
    }
  };
          ...
          {step === 'select' ? (
            ...
            <>
              ...
              <div style={{ fontSize:12, color:'var(--t3)', textAlign:'center', marginTop:-8 }}>
                Usá el teclado para ingresar tu PIN
              </div>
...
function _countPending(txt, approvalCol, nameCol, marcaCol) {
  try {
    const json = JSON.parse(txt.slice(txt.indexOf('{'), txt.lastIndexOf('}')+1));
    const hace30 = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
    return (json.table?.rows || []).filter(r => {
      const c = r.c || [];
      if (!c[nameCol]?.v || c[approvalCol]?.v === true) return false;
      if (marcaCol != null) {
        const marca = _parseGvizDate(c[marcaCol]?.v);
        if (marca && marca < hace30) return false;
      }
      return true;
    }).length;
  } catch(_) { return 0; }
...
function App() {
  ...
  const fetchBadges = useCallback(() => {
    Promise.all([
      ...
      const perm = permTxt ? _countPending(permTxt, 11, 1, 0) : 0;
      const falt = faltaTxt ? _countPending(faltaTxt, 7, 2, 0) : 0;
