...
/* ===== SIDEBAR ===== */
window.Sidebar = function Sidebar({ sections, active, onSelect, badges, open, onClose }) {
  const groups = [];
  const seen = new Set();
  sections.forEach(s => {
    if (!seen.has(s.group)) {
  ...
  return (
    <React.Fragment>
      {open && (
        <div onClick={onClose}
             style={{ position:'fixed', inset:0, zIndex:299, background:'rgba(0,0,0,0.55)' }} />
      )}
        ...
        <nav className="nav">
          {groups.map(({ group, items }) => (
            <div key={group || '_'} className="nav-group">
              ...
              {items.map(s => {
                const badgeCount = badges?.[s.id]?.total || 0;
                return (
                <div
                  key={s.id}
                ...
                >
                  ...
                  <span style={{
                    ...
                    background: s.live ? 'var(--ok)' : 'var(--warn)',
                    opacity: 0.55,
                  }} />
                </div>
              );})}
...
/* ===== HEADER ===== */
window.Header = function Header({ section, theme, onThemeToggle, user, onLogout, onMenuToggle }) {
  return (
    <div className="header">
      <button className="mob-menu-btn" onClick={onMenuToggle} aria-label="Menú">
      ...
      <div className="header-right">
        ...
        {user && (
          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
            <div className="header-user-name" style={{ textAlign:'right', lineHeight:1.3 }}>
              <div style={{ fontSize:13, fontWeight:600, color:'var(--t1)' }}>{user.name}</div>
            </div>
            <div className="avatar-sm"
                 style={{ background:'var(--accent)', color:'#fff', cursor:'default' }}>
              {user.initials}
            </div>
...
window.Select = function Select({ value, onChange, options }) {
  ...
  const handleToggle = () => {
    if (btnRef.current) {
      ...
      setCoord({ top: r.bottom + 4, left: r.left, minWidth: r.width });
    }
    setOpen(o => !o);
  };
    ...
    <React.Fragment>
      ...
      <div className="sel-dropdown"
           style={{ position:'fixed', top:coord.top, left:coord.left,
                    minWidth:coord.minWidth, zIndex:9001 }}>
        {options.map((o, i) => (
